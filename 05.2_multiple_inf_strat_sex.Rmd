...
runGeeglmParallel(datasets = c("female_imp_dat_bcep", "female_imp_dat_bdcipp", "female_imp_dat_dnbp", "female_imp_dat_dphp", "male_imp_dat_bcep", "male_imp_dat_bdcipp", "male_imp_dat_dnbp", "male_imp_dat_dphp"), 
                  ...
                  outcomes = c("ss_std_score_c", "com_raw_c", "coop_raw_c", "assert_raw_c","res_raw_c",
                              ...
                              "int_raw_p", "as_raw_p"),
                  outputFolder = "~/Documents/ope_ssis/result/gee_mi_res_march/strata_sex")
```
...
```{r}
extractGEEParallel(input_folder="~/Documents/ope_ssis/result/gee_mi_res_march/strata_sex", 
                          output_folder= "~/Documents/ope_ssis/result/gee_mi_res_march/strata_sex")
```

#=============================================================================#
#Preping results table
```{r}
df_res<- read_csv(paste0(res, "gee_mi_res_march/strata_sex/output.csv")) |>
  select(c(1,2,6,7,16,17)) |>
  mutate(
    gender = exposure,
    file_name = str_replace(outcome, "^imp_dat_", ""),
    exposure = str_extract(file_name, "^[^_]+") |> toupper(),
    file_name = str_remove(file_name, "^[^_]+_"),
    outcome_set = case_when(str_detect(file_name, "p$") ~ "Caregiver",
                            str_detect(file_name, "c$") ~ "Adolescent",
                            TRUE ~ NA_character_),
    outcome = str_remove(file_name, "_[cp]$"),
    term = case_when(
           term == "log(get(exposure))" ~ as.character(exposure),
           TRUE ~ paste0(as.character(exposure), "-", str_extract(term, "(?<=:)[^:]+$")))) |>
  select(-c(file_name)) |>
  mutate(outcome_label = fct_recode(outcome,
         ...
         outcome_category = case_when(outcome %in% 
...
x<- filtered_data |>
  filter(outcome_label == "Externalizing",
         outcome_set == "Caregiver")
```
...
```{r}
df_res_combined <- bind_rows(result_male, result_female)

write_csv(df_res_combined, paste0(res, "gee_mi_res_march/strata_sex/final_est_ci.csv"))
```
