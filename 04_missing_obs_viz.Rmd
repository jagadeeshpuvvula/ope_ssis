...
df<- read_csv(paste0(dat_loc, "analytic_set.csv")) |>
  rename_with(~ sub("postnatal_8_yr$", "postnatal_12_yr", .), ends_with("postnatal_8_yr")) |>
  rename_with(~ sub("postnatal_5_yr$", "postnatal_8_yr", .), ends_with("postnatal_5_yr")) |>
  rename_with(~ sub("postnatal_4_yr$", "postnatal_5_yr", .), ends_with("postnatal_4_yr"))|>
  rename_at(vars(contains("postnatal_")), ~ str_replace(., "postnatal_", "Child-")) |>
  select(-ends_with("Child-12_yr"))
```

#exp_dat from 01_exp_dat.rmd file line 87
```{r}
missing_data <- ifelse(is.na(df[, 1:33]), "BelowLOD", ifelse(df[, 1:37] == 9999, "NR-interf", FALSE))
